...
class MyAI(AI):
    ...
    def getAction(self, number: int) -> "Action Object":
        ...
        print(self.explored)
        if (self.lastX, self.lastY) not in self.mines:
        ...
        for i in range(self.row):
            for j in range(self.col):
                ...
                if self.remainingBoard[i][j] == self.effectiveBoard[i][j]:
        ...
        for i in range(self.row):
            for j in range(self.col):
                if self.effectiveBoard[i][j] is not False and self.effectiveBoard[i][j] == 0 and self.remainingBoard[i][j] != 0:
